...
def get_net(hardware_config=None, cate='vgg16', num_classes=10):
    ...
    for line in range(model_layer_num):
        layer = model[line, ...]
        if layer[5] == 1:  # conv
            layer_config_list.append({'type': 'conv', 'in_channels': layer[1], 'out_channels': layer[0], 'kernel_size': layer[3], 'padding': 1, 'stride': layer[4]})
            if layer[7] != 0:
                layer_config_list[-1]['input_index'] = [layer[7]]
            if layer[6] == 1:
                layer_config_list.append({'type': 'relu'})
            elif layer[6] >= 2:
                layer_config_list.append({'type': 'relu'})
                layer_config_list.append({'type': 'pooling', 'mode': 'MAX', 'kernel_size': layer[6], 'stride': layer[6]})
        elif layer[5] == 0:  # linear
            layer_config_list.append({'type': 'fc', 'in_features': layer[1], 'out_features': layer[0]})
            if layer[6] == 1:
                layer_config_list.append({'type': 'relu'})
            ...
            layer_config_list.append({'type': 'element_sum', 'input_index': [-1, layer[7]]})
